...
class Receiver { 
    public: 
        Receiver(int port, int window_size, std::string output_dir, std::string log_file);
        void startReceiving();
...
Receiver::Receiver(int port, int window_size, std::string output_dir, std::string log_file):
    ...
    while(true){
        ...
        if(p.header.type == 0){ //START
            ...
            if(connection){ //drops packet
                std::cout << "Packet1 dropped due to existing connection/transfer" << std::endl;
                continue;
            } else{
                connection = true; 
                next_seq = 0; 
...
int main(int argc, char* argv[]){
    ...
    Receiver w(port, window_size, output_dir, log_file);
    w.startReceiving();
    return -1; 
} 
