...
void SenderOpt::startConnection() {
    ...
    Packet startPacket(STARTOPT, {}, randSeqNum);  // Sequence number for START
...
void SenderOpt::sendData(const std::string& filename) {
    ...
    while (offset < buffer.size() || window->hasPackets()) {
        ...
        while (window->canAddPacket() && offset < buffer.size()) {
            ...
            Packet dataPacket(DATAOPT, chunk, dataSeqNum++);
        ...
        if (ackPacket.getType() == 4) {
            ...
            for (Packet& packet : window->getPackets()) {
...
void SenderOpt::endConnection() {
    Packet endPacket(ENDOPT, {}, randSeqNum);  // Sequence number for END should match START
...
void SenderOpt::sendPacket(Packet& packet, bool isFirstSend) {
...
bool SenderOpt::isAckValid(const Packet& ackPacket) {
